    ...
    methods: {
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].user_id) {
        ...
        SaveOffenderChanges() {
            $.post('http://mdt/saveOffenderChanges', JSON.stringify({
                ...
                id: this.offender_selected.user_id,
            ...
            this.offender_selected.photo = this.offender_changes.photo;
        ...
        AddCharge(id) {
            for (var key in this.offenses) {
                if (id == this.offenses[key].id) {
                    var offense_name = this.offenses[key].title
        ...
        RemoveCharge(offense) {
            for (var key in this.report_new.charges) {
                if (offense == key) {
                    ...
                    for (var key in this.offenses) {
                        if (offense == this.offenses[key].title) {
                            this.report_new.recommended_fine = this.report_new.recommended_fine - this.offenses[key].amount
                            if (this.offenses[key].jailtime) {
                                this.report_new.recommended_sentence = this.report_new.recommended_sentence - this.offenses[key].jailtime
                            }
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.user_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.desc) {
                $.post('http://mdt/submitNewReport', JSON.stringify({
                    title: this.report_new.title,
                    user_id: this.report_new.user_id,
                    name: this.report_new.name,
                    charges: this.report_new.charges,
                    desc: this.report_new.desc,
                ...
                this.report_new.desc = "";
                this.report_new.name = null;
                this.report_new.user_id = null;
        ...
        ToggleReportEdit() {
            if (this.report_edit.enable) {
                ...
                this.report_edit.data.desc = this.report_selected.desc;
            }
            return;
        },
        ...
        SaveReportEditChanges() {
            $.post('http://mdt/saveReportChanges', JSON.stringify({
                ...
                desc: this.report_edit.data.desc
            }));

            this.report_selected.title = this.report_edit.data.title;
            this.report_selected.desc = this.report_edit.data.desc;
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://mdt/submitNewWarrant', JSON.stringify({
                ...
                user_id: this.warrant_new.user_id,
        ...
        RemoveLicense(license) {
            for (var key in this.offender_selected.licenses) {
                ...
                if (license.title == license2.title) {
    ...
    computed: {
        filtered_offenses() {
            return this.offenses.filter(offense => {
                if (
                    offense.title.toLowerCase().search(this.report_new.charges_search.toLowerCase()) != -1
                    )
                    return offense;
            })
        },
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                mdtApp.offender_changes.photo = mdtApp.offender_selected.photo;
