...
RegisterCommand("cadastrarmulta",function(source,args)
	local nome = args[1]
	local multa = tonumber(args[2])
	local tempo = tonumber(args[3])
	if nome and multa and tempo and multa >= 0 and tempo >= 0 then
		TriggerServerEvent("mdt:insertFine", nome, multa, tempo)
	else
		TriggerEvent("Notify", "negado", "Valores inválidos")
	end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        -- if not isVisible and IsPedInAnyPoliceVehicle(playerPed) and IsControlJustPressed(0, 304) and GetEntitySpeed(playerVeh) < 10.0 then
...
RegisterNUICallback("getOffender", function(data, cb)
    TriggerServerEvent("mdt:getOffenderDetailsById", data.user_id)
    cb('ok')
end)
...
RegisterNUICallback("sentencePlayer", function(data, cb)
    ...
    TriggerServerEvent("mdt:sentencePlayer", data.jailtime, data.charges, data.user_id, data.fine, players)
    cb('ok')
end)
