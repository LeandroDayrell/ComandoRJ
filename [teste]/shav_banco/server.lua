...
vRPclient = Tunnel.getInterface("vRP","shav_banco")
isTransfer = false
server = {}
emP = {}

Tunnel.bindInterface("shav_banco",server)
emP = Tunnel.getInterface("shav_banco")
...
function server.depositar(amount)
	local source = source
  local user_id = vRP.getUserId(source)
  local banco = vRP.getBankMoney(user_id)
	if user_id then
    if amount > 0 then
      if vRP.tryDeposit(user_id,parseInt(amount)) then
        ...
        TriggerClientEvent("shav_banco:update", source)
      else
...
function server.transferencia(to ,amountt)
