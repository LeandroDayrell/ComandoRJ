...
-----------------------------------------------------------------------------------------------------------------------------------------
src = {}
Tunnel.bindInterface("frash_arena",src)
frash = Tunnel.getInterface("frash_arena")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local searchProgress = true
local isDominating = true
local dominatingPerms = {}
local hasPermName = ""

Citizen.CreateThread(function()
	while true do
		if isDominating ~= frash.isDominating() then
			isDominating = frash.isDominating()
			if isDominating then
				dominatingPerms = frash.dominatingNow()
				hasPermName = ""
				for kx, vx in pairs(frash.dominatingNow()) do
					if frash.checkPermission(vx) then
						hasPermName = vx
					end
				end
			else
							...
							if distance <= 1.8 then 
								...
								if IsControlJustPressed(0,38) and not searchProgress then
									...
									SetTimeout(10,function()
										...
										ClearPedTasks(ped)
										vRP._stopAnim(false)
										frash.searchChest(k, hasPermName)
