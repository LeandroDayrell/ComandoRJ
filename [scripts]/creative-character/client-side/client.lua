RegisterNetEvent("creative-character:normalSpawn")
RegisterNetEvent("creative-character:characterCreate")
...
AddEventHandler("creative-character:characterCreate",function()
	doStatus = 1
...
AddEventHandler("creative-character:normalSpawn",function(firstspawn)
	if firstspawn then
...
function changeGender(model)
	...
	if HasModelLoaded(mhash) then
		...
		SetEntityHealth(PlayerPedId(),230)
...
RegisterNUICallback('cDoneSave',function(data,cb)
	...
	TriggerServerEvent("creative-character:finishedCharacter",characterNome,characterSobrenome,characterAge,currentCharacterMode)
end)
