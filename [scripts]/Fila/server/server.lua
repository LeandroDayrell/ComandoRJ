...
Citizen.CreateThread(function()
    ...
    local function checkTimeOuts()
        ...
        while i <= Queue:GetSize() do
            ...
            elseif (data.timeout >= 30) and data.source ~= "debug" and os_time() - data.firstconnect > 1 then
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 30 and lastMsg >= 35000) or data.timeout >= 60) and data.source ~= "debug" and os_time() - data.firstconnect > 1 then
                Queue:RemoveFromQueue(data.source, true)
                Queue:RemoveFromConnecting(data.source, true)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] foi removido da fila por conta de um timeout")
            else
