                ...
                if distancia <= 50 then
                    if distancia <= 1.5 then
                        ...
                        if IsControlJustPressed(0,38) and not IsPedInAnyVehicle(ped,false) then
                            TriggerServerEvent('offred_carshop:abrirPainelpacorabane')
...
RegisterNetEvent('offred_carshop:abrirPainelpacorabane')
AddEventHandler('offred_carshop:abrirPainelpacorabane', function(id, nome, peso, estoque, preco, imagem)
...
RegisterNUICallback('comprarVeiculo', function(data)
    TriggerServerEvent('offred_carshop:comprarCarropacorabane', data.carro)
end)
