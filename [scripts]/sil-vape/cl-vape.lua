	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("~c~Você ~b~começou ~c~ a usar seu vape.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("~r~Você está segurando seu vape.")
			end
		else
			ShowNotification("~r~Você não pode fazer isso em um veículo.")
		end
	else
		ShowNotification("~r~Você não pode fazer isso morto.")
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("~c~Você ~r~parou~c~ de usar seu vape.")
	else
		ShowNotification("~r~Você não está segurando seu vape.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		ShowNotification("~r~Você tem que estar segurando seu vape para fazer isso.")
	end
end)
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Aperte ~b~\"E\"~c~ para vaporar.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Aperte ~b~\"G\"~c~ para colocar o vape em posição de uso.")
			end
		end
	  Wait(1)
	end
