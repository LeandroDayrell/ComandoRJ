...
colorPickerP.on(["color:change"], function (color) {
    $.post('http://nrp_mechanic/changeItem',JSON.stringify({type: 'color', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
// cor secundaria
...
colorPickerS.on([ "color:change"], function (color) {
    $.post('http://nrp_mechanic/changeItem',JSON.stringify({type: 'Scolor', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
// cor smoke
...
colorPickerSmoke.on([ "color:change"], function (color) {
    $.post('http://nrp_mechanic/changeItem',JSON.stringify({type: 'smoke', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
// cor neon
var colorPickerNeon = new iro.ColorPicker("#neonPicker", {
    width: 140,
    color: "rgb(255,255,255)",
    borderWidth: 1,
    borderColor: "#000",
...
colorPickerNeon.on([ "color:change"], function (color) {
    $.post('http://nrp_mechanic/changeItem',JSON.stringify({type: 'neon', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
...
const mec = {
    ...
    open: (event) => {
        ...
        $('#textPrice').html('<div style="font-size: 10px;position: absolute;top: -16px;">Valor do Item</div><span id="prefix">R$</span>'+00+',<span id="zeroCount">00</span>');
        $('#textmoney').html('<span id="prefix">R$</span>'+formatarNumero(event.playerMoney)+',<span id="zeroCount">00</span>');
    },
    close: () => {
        $('body').hide();
        $.post('http://nrp_mechanic/close')
    },
    acceptedEvents: {
        open(event) {
           mec.open(event.data) 
        ...
        update(event){
            ...
            $('#textPrice').html('<div style="font-size: 10px;position: absolute;top: -16px;">Valor do Item</div><span id="prefix">R$</span><span id="valMods">'+formatarNumero(data.totalPrice)+'</span>,<span id="zeroCount">00</span>');
        ...
        updateColors(event){
            ...
            if (data.neon.r) {
                colorPickerNeon.color.rgb = { r: data.neon.r, g: data.neon.g, b: data.neon.b };
            }
...
function acceptMod() {
    $("#warning").hide();
    $.post('http://nrp_mechanic/changeCamera',JSON.stringify({ type: 'reset' }) )
    $('.menu-option').hide();
    $('#home').fadeIn();
    $('.sectionItem').show();
    $('#textContent').text('Mecânica');
    ...
    submenu = false;
    $.post('http://nrp_mechanic/acceptMod',JSON.stringify({price: price}));
}
...
function declinetMod() {
    ...
    $('#textContent').text('Mecânica');
    submenu = false;
    $.post('http://nrp_mechanic/declineMod' )
}
...
function selectHome(e) {   
    ...
    if (haveItem == 'true') {
        ...
        var id = $(e).attr("href")
        $.post('http://nrp_mechanic/changeCamera', JSON.stringify({ type: id }) )
...
function selectItem(element) {
    ...
    $.post('http://nrp_mechanic/changeItem',JSON.stringify({ type: element.dataset.type, value: element.dataset.value, part: element.dataset.part, partId: element.dataset.custom, before: beforeInstalled }) )
    submenu = true;
}

function selectColor(type, op, colorP){
    $.post('http://nrp_mechanic/changeItem',JSON.stringify({ type: 'customcolor', customColor: type, op: op, colorP: colorP }) )
}

function setXenonLights(op){
    $.post('http://nrp_mechanic/changeItem',JSON.stringify({ type: 'xenoncolor', op: op}) )
}
