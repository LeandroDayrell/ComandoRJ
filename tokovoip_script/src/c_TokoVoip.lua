...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	...
	if (self.talking == 1 or self.plugin_data.radioTalking) then
		if not isTalking then
			isTalking = true
			TriggerEvent('voip:talking', isTalking)
		end
		info = "<font class='talking'>" .. info .. "</font>";
	elseif (self.talking == 0) then
		if isTalking then
			isTalking = false
			TriggerEvent('voip:talking', isTalking)
		end
	...
	if (self.plugin_data.radioChannel ~= -1 and self.myChannels[self.plugin_data.radioChannel]) then
		...
		TriggerEvent('voip:info', self.myChannels[self.plugin_data.radioChannel].name);
	end
	...
	Citizen.CreateThread(function()
		while (true) do
			...
			elseif (IsControlJustPressed(0, self.keyProximity)) then -- Switch proximity modes (normal / whisper / shout)
				...
				TriggerEvent('voip:mode', info);
				self:updateTokoVoipInfo();
			end

			if (IsControlPressed(0, self.radioKey) and self.plugin_data.radioChannel ~= -1) and not IsPlayingAnim() and GetEntityHealth(PlayerPedId()) > 100 then -- Talk on radio
...
local anims = {    
    { dict = "random@arrests@busted", anim = "idle_a" },            -- MÃOS NA CABEÇA
    { dict = "random@mugging3", anim = "handsup_standing_base" },   -- LEVANTAR A MAO
    { dict = "mp_arresting", anim = "idle" }                        -- ALGEMADO
}

function IsPlayingAnim()
    for id,anim in pairs(anims) do
        if IsEntityPlayingAnim(PlayerPedId(), anim.dict, anim.anim, 3) then
            return true
        end
    end
    return false
