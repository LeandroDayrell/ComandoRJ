...
local function clientProcessing()
	...
	for k,v in pairs(playerList) do
		...
		local playerPed = GetPlayerPed(player);
		if (playerPed and voip.serverId ~= playerServerId ) then
...
function addPlayerToRadio(channel)
	if channel <= 100 then
		TriggerEvent('voip:mhz', 'Frequencia: '..channel);
	elseif channel > 100 then
		TriggerEvent('voip:mhz', 'Chamada: '..channel);
	end
