...
Citizen.CreateThread(function()
	while true do
		...
		if emservico then
			...
			if distance <= 50.0 and IsVehicleModel(vehicle,GetHashKey("paramedicocharger2014")) then
...
Citizen.CreateThread(function()
	while true do
		...
		if emservico then
			...
			if IsControlJustPressed(0,168) and IsVehicleModel(vehicle,GetHashKey("paramedicocharger2014")) then
				RemoveBlip(blips)
				if DoesEntityExist(passageiro) then
					TaskLeaveVehicle(passageiro,vehicle,262144)
					TaskWanderStandard(passageiro,10.0,10)
