...
function vRP.getAllLicenses()
  local rows = vRP.query("vRP/getall")
  if #rows > 0 then
    return rows
  else
    return -1
		...
		if user_id then
			...
			if not vRP.isBanned(user_id) then
				 ...
				 if vRP.isWhitelisted(user_id) then
					...
					deferrals.done("CRJ ... SEU ID: "..user_id.. " ENTRE NO NOSSO DISCORD: Discord.gg/VUmdPh5")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Infelizmente você foi lixado de nossa cidade ID:"..user_id)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
