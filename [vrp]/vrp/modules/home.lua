...
vRP.prepare("vRP/set_address","REPLACE INTO vrp_user_homes(user_id,home,number,chest) VALUES(@user_id,@home,@number,'[]')")
...
local function build_entry_menu(user_id,home_name)
		...
		if huser_id then
			if huser_id == user_id then
				if not vRP.accessHome(user_id,home_name,number) then
					TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Aguarde um pouco, a residência está ocupada no momento.")
				end
			else
				...
				if hplayer ~= nil then
					...
					if vRP.request(hplayer,identitys.name.." "..identitys.firstname.." está tocando seu interfone, deseja abrir?",30) then
						vRP.accessHome(user_id,home_name,number)
					else
						TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Recusou sua entrada na residência.")
					end
				end
			end
		else
			TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"O interfone escolhido não foi encontrado.")
		end
	end }
	...
	menu["Comprar"] = { function(player,choice)
		...
		if not owner then
			...
			if number then
				if vRP.request(player,"Você deseja comprar esta casa?",30) then
					if vRP.tryPayment(user_id,home.buy_price) then
						vRP.setUserAddress(user_id,home_name,number)
						TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Você adquiriu uma residência.")
					else
						TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Dinheiro insuficiente.")
					end
				end
			else
				TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"A residência já tem um proprietário.")
			end
		else
			TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Você já é proprietário de uma residência.")
		end
	end,"<b>Valor:</b> "..vRP.format(parseInt(home.buy_price)) }

	menu["Vender"] = { function(player,choice)
		if vRP.request(player,"Você deseja vender sua casa?",30) then
			local addresses = vRP.getUserAddress(user_id)
			local owner = false
			for k,address in pairs(addresses) do
				if address and address.home == home_name then
			...
			if owner then
				...
				TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Você vendeu sua residência.")
			else
				TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Residência não encontrada.")
			end
		end
	end,"<b>Valor:</b> "..vRP.format(parseInt(home.sell_price)) }
