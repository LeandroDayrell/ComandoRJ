...
vRP.items = {}
function vRP.defInventoryItem(item, name, description, increase, weight, prop)
	if weight == nil then weight = 0 end
	if prop == nil then prop = 'prop_paper_bag_small' end
	vRP.items[item] = {name = name, description = description, increase = increase, weight = weight, prop = prop}
end
...
function vRP.getItemDefinition(idname)
	...
	if item then
		return vRP.computeItemName(item), vRP.computeItemWeight(item)
	end
	return nil,nil
end
