	...
	repeat
		...
		if IsPedDeadOrDying(ped) and not IsPedAPlayer(ped) and distance <= 1.5 and reviver == nil then
			...
			SetTimeout(15000,function()
				...
				vRP._stopAnim(false)
				TriggerServerEvent("crj:pagamento")
...
AddEventHandler('notificacao',function(x,y,z,user_id)
	local distance = GetDistanceBetweenCoords(x,y,z,218.88,1393.11,239.83,true)
	if distance <= 5000 then
		if not DoesBlipExist(blips[user_id]) then
			...
			SetBlipScale(blips[user_id],0.8)
			SetBlipSprite(blips[user_id],567)
		...
		if prisioneiro then
			...
			crjSleep = 1
				...
				if distance02 <= 1.2 then
					...
					if IsControlJustPressed(0,38) then
						reducaopenal = false
						TriggerServerEvent("diminuirpenacrj")
						vRP._DeletarObjeto()
					end
				end
			end
