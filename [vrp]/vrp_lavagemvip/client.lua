...
vRPlv = {}
Tunnel.bindInterface("vrp_lavagemvip",vRPlv)
Proxy.addInterface("vrp_lavagemvip",vRPlv)
LVserver = Tunnel.getInterface("vrp_lavagemvip")

cfg = module("vrp_lavagemvip", "cfg/configvip")
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
		for k, v in ipairs(cfg.lavagemvip) do
			...
			if Distancia < 6.0 then
				...
				TextoMarker(x,y,z+0.4, "APERTE ~r~[ F ]~w~ PARA LAVAR O DINHEIRO", Opacidade)
					DrawMarker(cfg.markervip.idmarker, x,y,z-0.99, 0, 0, 0, 0, 0, 0, cfg.markervip.x1,cfg.markervip.y1,cfg.markervip.z1,cfg.markervip.r,cfg.markervip.g,cfg.markervip.b,cfg.markervip.a, cfg.markervip.pula, 0, 0, cfg.markervip.gira)
				if contador == 0 then
					TextoMarker(x,y,z+0.1, "~r~Lavagem: ~g~".. (cfg.lavagemporcento)*100 .."%", Opacidade)
					if (IsControlJustPressed(1,49)) then
						if true then
							SetEntityCoords(player, cfg.lavagemcoordvip)
