...
vRPclient = Tunnel.getInterface("vRP","vrp_bancoitau")
isTransfer = false
UFserver = {}
emP = {}

Tunnel.bindInterface("vrp_bancoitau",UFserver)
emP = Tunnel.getInterface("vrp_bancoitau")
...
function UFserver.depositar(amount)
	local source = source
  local user_id = vRP.getUserId(source)
  local banco = vRP.getBankMoney(user_id)
	if user_id then
    if amount > 0 then
      if vRP.tryDeposit(user_id,parseInt(amount)) then
        ...
        TriggerClientEvent("vrp_bancoitau:update", source)
      else
...
function UFserver.saque(amount)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
function UFserver.multas(amount)
	local source = source
  local user_id = vRP.getUserId(source)
  local banco = vRP.getBankMoney(user_id)
  if user_id then
...
function UFserver.transferencia(to ,amountt)
