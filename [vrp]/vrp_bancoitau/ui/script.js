...
month[11] = "DEZEMBRO";
        ...
        if (valor > 0){
            switch(tipo){
                case "transferencia":
                    if(passaporte > 0){
                        $.post(
                            "http://vrp_bancoitau/transferencia",
                            JSON.stringify({
                              amount: valor,
                              nid: passaporte,
                            })
    ...
    document.onkeyup = function(data) {
        if (data.which == 27) {
          $.post("http://vrp_bancoitau/close", JSON.stringify({}), function(datab) {});
        }
      };
});