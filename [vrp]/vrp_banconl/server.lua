...
AddEventHandler("vRPclient:playerSpawned",function(user_id,source) 
    ...
    TriggerClientEvent('banking:updateBalancechick', source, bankbalance, multasbalance, walletbalance)
end)
...
function Depositar(user_id, amount)
  ...
  TriggerClientEvent("banking:updateBalancechick", source, new_balance)
...
RegisterServerEvent('bank:updatedup')
AddEventHandler('bank:updatedup', function()
  local source = source
  local user_id = vRP.getUserId(source)
  local bankbalance = vRP.getBankMoney(user_id)
  ...
  TriggerClientEvent("banking:updateBalancechick", source, bankbalance, walletbalance, multasbalance, identity.name.." "..identity.firstname) -- ADICIONAR VALORES AQUI PARA APARECER NA NUI + JAVA SCRIPT + HTML
end)

RegisterServerEvent('bank:pagarmultadup')
AddEventHandler('bank:pagarmultadup', function(amount)
  ...
  if vRP.tryFullPayment(user_id,rounded) then
    ...
    TriggerClientEvent("banking:updateBalancechick", source, bank, wallet,novamulta)
    TriggerClientEvent("banking:removeMultachick", source, rounded)
...
RegisterServerEvent('bank:depositdup')
    ...
    if amount and type(amount) == "number" then
      ...
      if (rounded > 0) then
        ...
        if (rounded <= wallet) then
          ...
          TriggerClientEvent("banking:updateBalancechick", source, new_balance,carteira)
          TriggerClientEvent("banking:addBalancechick", source, rounded)
...
RegisterServerEvent('bank:withdrawdup')
  ...
  if user_id then
    if amount and type(amount) == "number" then
      ...
      if (rounded <= bankbalance) then
        ...
        -- Update NUI
        TriggerClientEvent("banking:updateBalancechick", source, new_balance, carteira)
        TriggerClientEvent("banking:removeBalancechick", source, rounded)
        -- Salva o extrato
...
RegisterServerEvent('bank:quickCashdup')
AddEventHandler('bank:quickCashdup', function()
  local source = source
  local user_id = vRP.getUserId(source)
  ...
  if (bankbalance >= quantia) then
    ...
    local carteira = vRP.getMoney(user_id)
    TriggerClientEvent("banking:updateBalancechick", source, new_balance,carteira)
    TriggerClientEvent("banking:removeBalancechick", source, quantia)
...
RegisterServerEvent('bank:transferdup')
AddEventHandler('bank:transferdup', function(toPlayer, amount)
  local source = source
  local user_id = vRP.getUserId(source)
      ...
      if (rounded > 0) then
        ...
        if (rounded <= bankbalance) then
          ...
          if player then -- Está online
            ...
            TriggerClientEvent("banking:updateBalancechick", player, newBalance_Player)
            TriggerClientEvent("banking:addBalancechick", player, rounded)
          else
          ...
          local carteira = vRP.getMoney(user_id)
          TriggerClientEvent("banking:updateBalancechick", source, newBalance, carteira)
          TriggerClientEvent("banking:removeBalancechick", source, rounded)
          -- Extrato
