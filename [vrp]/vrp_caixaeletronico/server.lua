...
function func.checkRobbery(id,x,y,z,head)
	...
	if user_id then
		...
		elseif (os.time()-timers) <= 900 then
			TriggerClientEvent("Notify",source,"aviso","Os caixas estão vazios, aguarde <b>"..vRP.format(parseInt((900-(os.time()-timers)))).." segundos</b> até que os civis depositem dinheiro.")
		else
			andamento = true
			timers = os.time()
			dinheirosujo = {}
			...
			for l,w in pairs(policia) do
				...
				if player then
					async(function()
						...
						TriggerClientEvent('chatMessage',player,"911",{65,130,255},"O ID: "..user_id.." iniciou o roubo no ^1Caixa Eletrônico^0, dirija-se até o local e intercepte os assaltantes.")
					end)
				end
			end
