...
$(document).ready(() => {
    ...
    window.addEventListener('message', function(e) {
        ...
        if (data.action == 'basic') {
            if (data.status) {
                hudp = true;
                $('.health > .blur, .health > img, .armor > .blur, .armor > img').hide();
                $('.health > div:nth-child(1), .armor > div:nth-child(1)').fadeIn('slow');
            }else if (!data.status) {
                hudp = false;
                $('.health > div:nth-child(1), .armor > div:nth-child(1)').hide();
                $('.health > .blur, .health > img, .armor > .blur, .armor > img').fadeIn('slow');
            }
        }
        ...
        if (data.action == 'player') {            
            if (health != parseInt(data.health)) {
                health = data.health;
                $('.health > div:nth-child(1) > span:nth-child(1)').html((health < 0) ? '0%' : health + '%');
                if (!hudp && !hhealth) {
                    hhealth = true;
                    $('.health > .blur').hide();
                    $('.health > img').hide();
                    $('.health > div:nth-child(1)').fadeIn('slow');
                    setTimeout(() => {
                        $('.health > div:nth-child(1)').fadeOut('fast');
                        $('.health > .blur').fadeIn('fast');
                        setTimeout(() => {
                            $('.health > img').fadeIn('slow');
                            hhealth = false;
                        }, 300);
                    }, 3500);
                }
            ...
            if (armor != parseInt(data.armor)) {
                armor = data.armor;
                $('.armor > div:nth-child(1) > span:nth-child(1)').html((armor < 0) ? '0%' : armor + '%');
                if (!hudp && !harmor) {
                    harmor = true;
                    $('.armor > .blur').hide();
                    $('.armor > img').hide();                        
                    $('.armor > div:nth-child(1)').fadeIn('slow');
                    setTimeout(() => {
                        $('.armor > div:nth-child(1)').fadeOut('fast');
                        $('.armor > .blur').fadeIn('fast');
                        setTimeout(() => {
                            $('.armor > img').fadeIn('slow');
                            harmor = false;
                        }, 300);
                    }, 3500);
                }
