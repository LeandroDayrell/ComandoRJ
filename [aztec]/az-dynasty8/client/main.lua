...
Citizen.CreateThread(function()
	while true do
		...
		local distance = #(plyCoords - vector3(-128.48, -642.02, 168.84))
		if distance < 5 then
			sleep = 5
...
Citizen.CreateThread(function()
	while true do
		if not permission then
			permission = vAZserver.hasPermission()
			Citizen.Wait(15000)
		else
			Citizen.Wait(60000)
		end		
...
AddEventHandler('az-dynasty8:item', function()
	local permission = vAZserver.hasPermission()
	if permission and not vAZserver.inService() then
		TriggerEvent('Notify', 'negado', 'Você precisa estar em serviço para acessar o sistema!')
		return
	end
	if vAZserver.CheckItem() then
		...
		SendNUIMessage({
			action = "open",
			salesman = GetPlayerServerId(NetworkGetPlayerIndexFromPed(PlayerPedId())),
			permission = permission,
			types = vAZserver.GetHomeTypes()
		})
	end
end)
